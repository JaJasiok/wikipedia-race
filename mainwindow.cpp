...
MainWindow::MainWindow(QWidget *parent, QTcpSocket *socket, QString username) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(ui->webEngineView, &QWebEngineView::urlChanged, [this](const QUrl &url) {
        ...
        if(newUrl.startsWith("https://en.wikipedia.org/wiki/") &&             !newUrl.startsWith("https://en.wikipedia.org/wiki/Main_Page")&&
                !newUrl.startsWith("https://en.wikipedia.org/wiki/Wikipedia:") &&
                !newUrl.startsWith("https://en.wikipedia.org/wiki/Portal:") &&
                !newUrl.startsWith("https://en.wikipedia.org/wiki/Help:") &&
                !newUrl.startsWith("https://en.wikipedia.org/wiki/Special:"))
        {
            this->currentUrl = newUrl;
            MainWindow::sendData(newUrl.toLatin1());
//            qDebug() << "New URL:" << newUrl;
        }
        else if(newUrl.startsWith("data:")){

        }
        else{
            ui->webEngineView->setUrl(QUrl(currentUrl));
        }
    });
